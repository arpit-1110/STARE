...
class SVM_Classifier():
    def __init__(self,linear=True,kernel=linearKernel):
        ...
        self.classifier = svm.LinearSVC(max_iter=5000, class_weight = {-1.0:0.14, 1.0:1.63}, verbose=1)
        ...
        # if not linear:
        #     self.feature_map = Nystroem(gamma=0.2,random_state=42,n_components=50)
    ...
    def processData(self):
        ...
        for i in range(len(imageList)):
            ...
            labels = labels.reshape(-1,1)/255.0
            if i==0:
                self.X_train = feats
                self.Y_train = labels
            else:
                self.X_train = np.vstack((self.X_train,feats))
                self.Y_train = np.vstack((self.Y_train,labels))
        ...
        for i in [len(imageList)-2, len(imageList)-1]:
            ...
            if i==len(imageList)-2:
                ...
                self.Y_test = np.vstack((self.Y_test,labels))
...
if __name__ == "__main__":
    ...
    svm_classifier.predict()
    imagePath = '../data/images/im0001.ppm'
